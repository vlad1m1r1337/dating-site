...
async def create_user(db, body: dict):
    try:
        body = sanitize_body(body, ["username", "firstName", "lastName", "email"])
...
async def login_user(db, body: dict):
    try:
        ...
        # Удаляем истёкшие токены пользователя
        await db.execute(
            """DELETE FROM token WHERE user_id = $1 AND last_activity < $2""",
            user["id"],
            datetime.datetime.now().timestamp() - 3600 * 6,
        )
        # Ограничение: максимум 5 активных сессий, удаляем самую старую
        sessions = await db.fetch(
            """SELECT id FROM token WHERE user_id = $1 ORDER BY last_activity ASC""",
            user["id"],
        )
        if len(sessions) >= 5:
            await db.execute(
                """DELETE FROM token WHERE id = $1""", sessions[0]["id"]
            )
...
async def update_user(db, user, body):
    try:
        body = sanitize_body(body, ["firstName", "lastName", "email", "bio"])
        if check_email(body["email"]) is not None:
            return check_email(body["email"])
        if check_first_name(body["firstName"]) is not None:
            return check_first_name(body["firstName"])
...
async def like(db, origin, recipient):
    try:
        ...
        if await is_liked(db, recipient, origin) and await is_liked(
            ...
            await db.execute(
                """INSERT INTO chat (id, user_1, user_2)
                   SELECT $1, $2, $3
                   WHERE NOT EXISTS (
                       SELECT 1 FROM chat
                       WHERE (user_1 = $2 AND user_2 = $3)
                          OR (user_1 = $3 AND user_2 = $2)
                   )""",
                str(uuid.uuid4()),
                origin["id"],
                recipient["id"],
